...
UINT usbrdr_add_init_handle_data(void* pInitHandle, void* pUserData)
{
	if (!g_InitHandles)
...
void* usbrdr_get_init_handle_data(void* pInitHandle)
{
	return ListDictionary_GetItemValue(g_InitHandles, pInitHandle);
}

void usbrdr_remove_init_handle_data(void* pInitHandle)
{
...
UINT usbrdr_add_open_handle_data(DWORD openHandle, void* pUserData)
{
...
void* usbrdr_get_open_handle_data(DWORD openHandle)
{
	void* pOpenHandle = (void*)(size_t)openHandle;

	return ListDictionary_GetItemValue(g_OpenHandles, pOpenHandle);
}

void usbrdr_remove_open_handle_data(DWORD openHandle)
{
	void* pOpenHandle = (void*)(size_t)openHandle;
...
usbrdr_send(usbrdrPlugin* usbrdr, const void *data, size_t len)
{
	...
	status = usbrdr->channelEntryPoints.pVirtualChannelWrite(usbrdr->OpenHandle,
		Stream_Buffer(s), (UINT32) Stream_GetPosition(s), s);

	if (status != CHANNEL_RC_OK)
	{
		Stream_Free(s, TRUE);
		WLog_ERR(TAG,  "VirtualChannelWrite failed with %s (0x%08X)",
...
static VOID VCAPITYPE usbrdr_virtual_channel_open_event(DWORD openHandle, UINT event,
		LPVOID pData, UINT32 dataLength, UINT32 totalLength, UINT32 dataFlags)
{
	usbrdrPlugin* usbrdr;
	UINT error = CHANNEL_RC_OK;

	usbrdr = (usbrdrPlugin*) usbrdr_get_open_handle_data(openHandle);

	if (!usbrdr)
	{
		WLog_ERR(TAG, "usbrdr_virtual_channel_open_event: error no match");
		return;
	}
	...
	if (error && usbrdr->rdpcontext)
		setChannelError(usbrdr->rdpcontext, error, "usbrdr_virtual_channel_open_event reported an error");
...
static UINT usbrdr_virtual_channel_event_connected(usbrdrPlugin* usbrdr, const char *hostname, size_t length)
{
	...
	status = usbrdr->channelEntryPoints.pVirtualChannelOpen(usbrdr->InitHandle,
		&usbrdr->OpenHandle, usbrdr->channelDef.name, usbrdr_virtual_channel_open_event);

	if (status != CHANNEL_RC_OK)
	{
		WLog_ERR(TAG, "pVirtualChannelOpen failed with %s (0x%08X)",
				 WTSErrorToString(status), status);
		return status;
	}

	if ((error = usbrdr_add_open_handle_data(usbrdr->OpenHandle, usbrdr)))
	{
		WLog_ERR(TAG, "usbrdr_add_open_handle_data failed with error %lu", error);
		return error;
	}
	...
	if (length)
	{
		strncpy(usbrdr->server_name, hostname, (length >= sizeof(usbrdr->server_name)) ? sizeof(usbrdr->server_name)-1 : length);
	}
	else
	{
		strcpy(usbrdr->server_name, "RDP");
	}
...
static UINT usbrdr_virtual_channel_event_disconnected(usbrdrPlugin* usbrdr)
{
	...
	rc = usbrdr->channelEntryPoints.pVirtualChannelClose(usbrdr->OpenHandle);

	if (rc != CHANNEL_RC_OK)
	{
		WLog_ERR(TAG, "VirtualChannelClose failed with %s (0x%08x)",
				 WTSErrorToString(rc), rc);
		return rc;
	}

	usbrdr_remove_open_handle_data(usbrdr->OpenHandle);
...
{
	usbrdr_remove_init_handle_data(usbrdr->InitHandle);
...
static VOID VCAPITYPE usbrdr_virtual_channel_init_event(LPVOID pInitHandle, UINT event, LPVOID pData, UINT dataLength)
{
	usbrdrPlugin* usbrdr;
	UINT error = CHANNEL_RC_OK;

	usbrdr = (usbrdrPlugin*)usbrdr_get_init_handle_data(pInitHandle);

	if (!usbrdr)
	{
		WLog_ERR(TAG,  "usbrdr_virtual_channel_init_event: error no match");
		return;
	}
	...
	if (error && usbrdr->rdpcontext)
		setChannelError(usbrdr->rdpcontext, error, "usbrdr_virtual_channel_init_event reported an error");
}
...
BOOL VCAPITYPE VirtualChannelEntry(PCHANNEL_ENTRY_POINTS pEntryPoints)
{
	...
	if (usbrdr_rdp)
	{
			...
			(pEntryPointsEx->MagicNumber == FREERDP_CHANNEL_MAGIC_NUMBER))
		{
			...
			CopyMemory(&(usbrdr_rdp->channelEntryPoints), pEntryPoints, sizeof(CHANNEL_ENTRY_POINTS_FREERDP));
		} else
		{
			CopyMemory(&(usbrdr_rdp->channelEntryPoints), pEntryPoints, sizeof(CHANNEL_ENTRY_POINTS));
		}

		rc = usbrdr_rdp->channelEntryPoints.pVirtualChannelInit(
			&usbrdr_rdp->InitHandle,
			&usbrdr_rdp->channelDef, 1,
			VIRTUAL_CHANNEL_VERSION_WIN2000,
			usbrdr_virtual_channel_init_event);

		if (rc == CHANNEL_RC_OK)
		{
			usbrdr_add_init_handle_data(usbrdr_rdp->InitHandle, (void*)usbrdr_rdp);
		} else
		{
			WLog_ERR(TAG, "VirtualChannelInit(RDP) failed with %s (0x%08x)",
				WTSErrorToString(rc), rc);
			free(usbrdr_rdp);
			usbrdr_rdp = NULL;
		}
	...
	if (usbrdr_ts)
	{
			...
			(pEntryPointsEx->MagicNumber == FREERDP_CHANNEL_MAGIC_NUMBER))
		{
			...
			CopyMemory(&(usbrdr_ts->channelEntryPoints), pEntryPoints, sizeof(CHANNEL_ENTRY_POINTS_FREERDP));
		} else
		{
			CopyMemory(&(usbrdr_ts->channelEntryPoints), pEntryPoints, sizeof(CHANNEL_ENTRY_POINTS));
		}

		rc = usbrdr_ts->channelEntryPoints.pVirtualChannelInit(
			&usbrdr_ts->InitHandle,
		...
		if (rc == CHANNEL_RC_OK)
		{
			usbrdr_add_init_handle_data(usbrdr_ts->InitHandle, (void*)usbrdr_ts);
		} else
		{
